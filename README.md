...
1. Enable or disable [semantic reranking](https://learn.microsoft.com/azure/search/semantic-search-overview) using the `AZURE_SEARCH_SERVICE_SEMANTIC_SEARCH_SKU` variable. Run `azd env set AZURE_SEARCH_SERVICE_SEMANTIC_SEARCH_SKU disabled` to disable semantic ranking. Run `azd env set AZURE_SEARCH_SERVICE_SEMANTIC_SEARCH_SKU free` or `azd env set AZURE_SEARCH_SERVICE_SEMANTIC_SEARCH_SKU standard` to enable semantic ranking. By default, the free semantic ranking plan is enabled. The standard plan is billed by usage. Semantic ranking must be disabled if you are deploying the app in a [region that doesn't support it](https://learn.microsoft.com/azure/search/semantic-how-to-enable-disable?tabs=enable-portal#enable-semantic-ranking).
